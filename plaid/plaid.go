...
var (
	...
	PLAID_REDIRECT_URI                       = ""
	APP_PORT                                 = ""
	ProductionClient        *plaid.APIClient = nil
	SandboxClient           *plaid.APIClient = nil
)
...
func InitPlaid() {
	...
	if PLAID_CLIENT_ID == "" || PLAID_SECRET_PRODUCTION == "" || PLAID_SECRET_SANDBOX == "" {
	...
	// create Plaid client
	sandbox_configuration := plaid.NewConfiguration()
	sandbox_configuration.AddDefaultHeader("PLAID-CLIENT-ID", PLAID_CLIENT_ID)
	sandbox_configuration.AddDefaultHeader("PLAID-SECRET", PLAID_SECRET_SANDBOX)
	sandbox_configuration.UseEnvironment(plaid.Sandbox)
	SandboxClient = plaid.NewAPIClient(sandbox_configuration)
	...
	production_configuration := plaid.NewConfiguration()
	production_configuration.AddDefaultHeader("PLAID-CLIENT-ID", PLAID_CLIENT_ID)
	production_configuration.AddDefaultHeader("PLAID-SECRET", PLAID_SECRET_PRODUCTION)
	production_configuration.UseEnvironment(plaid.Production)
	ProductionClient = plaid.NewAPIClient(production_configuration)
	log.Printf("Plaid client initialized")
}

func CreateLinkToken(userIdInt int, isSandbox bool) (string, error) {
...
func ExchangePublicToken(publicToken string, userIdInt int, isSandbox bool) (string, string, error) {
	var Client *plaid.APIClient
	if isSandbox {
		Client = SandboxClient
	} else {
...
func GetTransactions(accessToken string, startDate string, endDate string, isSandbox bool) ([]plaid.Transaction, error) {
	var Client *plaid.APIClient
	if isSandbox {
		Client = SandboxClient
	} else {
...
func GetAccounts(accessToken string, isSandbox bool) ([]plaid.AccountBase, error) {
	var Client *plaid.APIClient
	if isSandbox {
		Client = SandboxClient
	} else {
