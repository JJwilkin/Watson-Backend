...
func AuthMiddleware(c *gin.Context) (int, bool, error) {
	...
	if authHeader == "" {
		...
		return -1, false, errors.New("authorization header required")
	}
	...
	if len(authHeader) < 7 || authHeader[:7] != "Bearer " {
		...
		return -1, false, errors.New("invalid authorization format. Use 'Bearer <token>'")
	}
	...
	tokenUserID, isSandbox, err := VerifyJWT(tokenString)
	if err != nil {
		...
		return -1, false, errors.New("invalid or expired token")
	}

	log.Printf("AuthMiddleware: Authentication successful for user ID: %d", tokenUserID)
	return tokenUserID, isSandbox, nil
}
